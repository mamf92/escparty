## Plan: Prep repo for robust AI guidance, deployment, Zustand, and single-player fixes
...
2. CI/CD and tests (main phase, *blocks deployment until basic CI exists*)
   ...
   2. Add a deploy workflow (`.github/workflows/deploy.yml`) that runs on push to `main` and deploys to chosen host (Vercel or GitHub Pages). If Github Pages: use existing `gh-pages` script; if Vercel/Netlify: configure build & env secrets. Recommend Vercel for automatic PR previews and simple env secret handling.
   ...
   5. Branch protection: enforce status checks (CI & tests) before merge to `main` and require PR review.

3. Zustand plan (design & incremental rollout, *parallelizable with testing setup*)
...
4. Reopen single-player quiz (quick UX + routing fix)
   1. Fix Home buttons to link to `/select-difficulty` instead of `/quiz` (placeholder). Update `src/pages/Home.tsx` to point the single-player button to [src/pages/SelectDifficulty.tsx](src/pages/SelectDifficulty.tsx).
...
**Relevant files**
- [.github/copilot-instructions.md](.github/copilot-instructions.md) — extend with required project facts
- [package.json](package.json) — scripts to reference (`dev`, `build`, `deploy`)
- [vite.config.ts](vite.config.ts) — `base` handling for GitHub Pages
- [src/firebase.ts](src/firebase.ts) — env keys required and emulator logic
- [src/utils/roomsFirestore.ts](src/utils/roomsFirestore.ts) — core multiplayer DB API, add integration tests here
- [src/components/Quiz.tsx](src/components/Quiz.tsx) — scoring logic and where to integrate store
- [src/pages/MultiplayerLobby.tsx](src/pages/MultiplayerLobby.tsx), [src/pages/Lobby.tsx](src/pages/Lobby.tsx), [src/pages/HostObserverView.tsx](src/pages/HostObserverView.tsx), [src/pages/MidQuizScoreboard.tsx](src/pages/MidQuizScoreboard.tsx)
- [src/pages/Home.tsx](src/pages/Home.tsx), [src/pages/SelectDifficulty.tsx](src/pages/SelectDifficulty.tsx)
...
If you approve this plan, I will: 1) update `.github/copilot-instructions.md` draft; 2) scaffold GitHub Actions CI and a basic test; 3) create a `useGameStore.ts` sketch and list the exact migration steps; 4) patch `Home.tsx` to point to select-difficulty. Tell me which of the four tasks you'd like me to start implementing first.